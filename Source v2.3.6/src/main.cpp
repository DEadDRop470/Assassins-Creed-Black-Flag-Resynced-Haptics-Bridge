// ACBFHapticsBridge v8-r9g 500 Hz input/performance cleanup revision
...
static DWORD WINABI worker_thread(void*) {
    ...
    load_config();
    {
        ...
        if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(b, '['); tb_time(b, st);
        tb_str(b, "] ACBFHapticsBridge v8-r9g-500hz-input-perf-cleanup starting gameBase=0x"); tb_hex(b, (u64)g_gameBase);
    ...
    }
    {
        ...
        SYSTEMTIME st{};
        if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(b, '['); tb_time(b, st); tb_str(b, "] v8-r9g 500Hz input + GameInput/menu/sink performance cleanup + r9f tuning retained legacyWalkLoopMs=");
    ...
    log_simple("v8-r9g routing: input worker polls every 2 ms (500 Hz); GameInput device identity/status/raw-button layout are cached; semantic menu mode bypasses analog/D-pad heuristics; ordinary QuadAudio sink pointer rebounds no longer cancel/reset/log; native combat/menu amplification shares one buffer gain pass; conventional rumble writes are capped at 4 ms cadence while start/stop remains immediate; F12 truly suspends controller polling for A/B diagnostics; all r9f tuning, r9d XInput 300 ms zero-reset, r9c L3-gated water lunge, r9 step-running and native-combat supplement retained");
    ...
    while (atomic_load_u32(&g_running)) {
        ...
        if (hotkey_pressed(VK_F9, f9Down) && atomic_load_u32(&g_enabled)) {
            schedule_configured_effect(g_map.dodge, 1.0f, g_cfg.fxDodge,
                                       ...
                                       g_cfg.dodgeStartTrimMs,
                                       g_cfg.semanticDodgeDurationMs);
            log_simple("F9 preview: v8-r9g semantic dodge trim/duration");
        }
