// ACBFHapticsBridge v8-r9k / v2.3.7.3 Ubisoft-isolated output compatibility revision
...
struct Config {
    ...
    // Steam TU 1.0.6 Wwise PostEvent producer. INI-overridable so a moved
    // executable can be re-targeted without a rebuild; the prologue signature
    // is still verified before any patch.
    u64 postEventRva = 0x0091DEA0ull;
    u32 enabled = 1;
    u32 inputBridge = 1;
...
// The Steam TU 1.0.6 PostEvent RVA (0x0091DEA0) now lives in Config::postEventRva
// and can be overridden from [Hook] PostEventRVA in the INI.
...
static void load_config() {
    ...
    g_cfg.postEventRva = read_ini_u64(L"Hook", L"PostEventRVA", g_cfg.postEventRva);
...
// v8-r9k / v2.3.7.3 Ubisoft-only direct DualSense fallback.
...
static bool install_flush_hook() {
    ...
    *(u64*)(patch + k) = (u64)&flush_hook; k += 8;
    ...
    atomic_store_u32(&g_hookInstalled, 1);
    {
        ...
        tb_hex(b, g_cfg.flushRva); tb_endline(b);
        append_log(b.bytes, b.len);
    }
    return true;
}
...
static bool install_v8_post_event_hook() {
    ...
    if (!g_gameBase || !g_cfg.postEventRva ||
        g_cfg.postEventRva + 6 > g_gameSize) return false;
    static const u8 signature[6] = {0x41,0x57,0x41,0x56,0x41,0x55};
    u8* target = g_gameBase + g_cfg.postEventRva;
    ...
    atomic_store_u32(&g_v8PostHookInstalled, 1);
    {
        ...
        tb_hex(b, g_cfg.postEventRva); tb_endline(b);
        append_log(b.bytes, b.len);
    }
    return true;
}
...
// Non-invasive probe: true when the vtable slot currently holds
// ImageBase+expectedFunctionRva (the same fail-closed test patch_state_vtable
// applies before writing).
static bool state_slot_matches(u64 slotRva, u64 expectedFunctionRva) {
    if (!g_gameBase || slotRva + sizeof(u64) > g_gameSize ||
        expectedFunctionRva >= g_gameSize) return false;
    return *(u64*)(g_gameBase + slotRva) == (u64)(g_gameBase + expectedFunctionRva);
}

// Executable-profile dispatch: try the Steam TU 1.0.6 pair, then the Steam
// TU 1.0.7 pair. When neither slot verifies, the 1.0.6 attempt is run anyway
// so the mismatch is logged and counted; nothing is ever patched on a
// mismatch.
static bool patch_state_vtable2(u64 slotRva106, u64 fnRva106,
                                u64 slotRva107, u64 fnRva107,
                                StateVirtualFn replacement, StateVirtualFn* original,
                                const char* label) {
    if (state_slot_matches(slotRva106, fnRva106))
        return patch_state_vtable(slotRva106, fnRva106, replacement, original, label);
    if (state_slot_matches(slotRva107, fnRva107))
        return patch_state_vtable(slotRva107, fnRva107, replacement, original, label);
    return patch_state_vtable(slotRva106, fnRva106, replacement, original, label);
}
...
static void install_gameplay_state_hooks() {
    ...
    // Slot RVAs and original-function RVAs are per exact executable
    // fingerprint: first pair Steam TU 1.0.6 (SHA-256 8d522381...), second
    // pair Steam TU 1.0.7 (SHA-256 614dab4a...), derived by byte-diffing the
    // two executables (unique fuzzy prologue match per function, slot found
    // as the unique data qword holding ImageBase+function, enter/exit
    // adjacency and vtable-neighbor code verified). Each pair is still
    // verified fail-closed at runtime before patching.
    patch_state_vtable2(0x09151C18ull, 0x015273A0ull, 0x091E0E98ull, 0x01521C20ull, &state_air_enter, &g_origAirEnter, "InAir enter");
    patch_state_vtable2(0x09151C20ull, 0x017FC510ull, 0x091E0EA0ull, 0x017FC1E0ull, &state_air_exit, &g_origAirExit, "InAir exit");
    patch_state_vtable2(0x09152898ull, 0x05E7F6C0ull, 0x091E1B18ull, 0x05F1D4C0ull, &state_jump_spot, &g_origJumpOnSpot, "JumpOnSpot enter");
    patch_state_vtable2(0x09152848ull, 0x06B118E0ull, 0x091E1AC8ull, 0x06BAF660ull, &state_free_jump, &g_origFreeJump, "FreeJump enter");
    patch_state_vtable2(0x0914E4E8ull, 0x02750BF0ull, 0x091DD768ull, 0x027D5940ull, &state_targeted_jump, &g_origTargetedJump, "TargetedJump enter");
    patch_state_vtable2(0x0914E3F8ull, 0x01FCD920ull, 0x091DD678ull, 0x01FA4D60ull, &state_freefall, &g_origFreefall, "Freefall enter");
    patch_state_vtable2(0x09151F38ull, 0x06B01790ull, 0x091E11B8ull, 0x06B9F510ull, &state_ledge_fall_enter, &g_origLedgeFallEnter, "FallFromLedge enter");
    patch_state_vtable2(0x09151F40ull, 0x06B01A30ull, 0x091E11C0ull, 0x06B9F7B0ull, &state_ledge_fall_exit, &g_origLedgeFallExit, "FallFromLedge exit");
    patch_state_vtable2(0x0914E218ull, 0x06AFDB60ull, 0x091DD498ull, 0x06B9B8E0ull, &state_transition_exit, &g_origTransitionExit, "TransitionExit enter");
    patch_state_vtable2(0x09147A38ull, 0x0152D180ull, 0x091D6CB8ull, 0x01527A00ull, &state_climb_a_enter, &g_origClimbAEnter, "Climb A enter");
    patch_state_vtable2(0x09147A40ull, 0x0102CC10ull, 0x091D6CC0ull, 0x0102B6F0ull, &state_climb_a_exit, &g_origClimbAExit, "Climb A exit");
    patch_state_vtable2(0x0920DC90ull, 0x06A98A70ull, 0x0929D130ull, 0x06B367F0ull, &state_climb_b_enter, &g_origClimbBEnter, "Climb B enter");
    patch_state_vtable2(0x0920DC98ull, 0x06A99E00ull, 0x0929D138ull, 0x06B37B80ull, &state_climb_b_exit, &g_origClimbBExit, "Climb B exit");
    patch_state_vtable2(0x09159718ull, 0x027D3CF0ull, 0x091E89B8ull, 0x02859AA0ull, &state_swim_a_enter, &g_origSwimAEnter, "Swimming A enter");
    patch_state_vtable2(0x09159720ull, 0x018A3D00ull, 0x091E89C0ull, 0x018A05B0ull, &state_swim_a_exit, &g_origSwimAExit, "Swimming A exit");
    patch_state_vtable2(0x091A3928ull, 0x08065C00ull, 0x09232D28ull, 0x080F8A70ull, &state_swim_b_enter, &g_origSwimBEnter, "Swimming B enter");
    patch_state_vtable2(0x091A3930ull, 0x08066540ull, 0x09232D30ull, 0x080F93B0ull, &state_swim_b_exit, &g_origSwimBExit, "Swimming B exit");
    patch_state_vtable2(0x0915AB98ull, 0x06C9ED00ull, 0x091E9E38ull, 0x06D3CB40ull, &state_shallow_enter, &g_origShallowEnter, "ShallowSwim enter");
    patch_state_vtable2(0x0915ABA0ull, 0x06C9FAC0ull, 0x091E9E40ull, 0x06D3D900ull, &state_shallow_exit, &g_origShallowExit, "ShallowSwim exit");
    patch_state_vtable2(0x09145208ull, 0x01B6C060ull, 0x091D4488ull, 0x01B6C0C0ull, &state_haystack_enter, &g_origHaystackEnter, "HayStack enter");
    patch_state_vtable2(0x09145210ull, 0x054F1850ull, 0x091D4490ull, 0x0559F190ull, &state_haystack_exit, &g_origHaystackExit, "HayStack exit");
...
// v8-r9k: dynamically resolve XInputSetState from the game's PE import table,
...
static u32 r9k_profile_from_xinput_rva(u64 rva) {
    ...
    // Steam TU 1.0.7 (SHA-256 614dab4a..., 469524824 bytes): XINPUT9_1_0.dll
    // XInputSetState IAT slot from the PE import directory.
    if (rva == 0x1C6109E0ull) return R9K_PROFILE_STEAM_FAMILY;
...
static u64* r9k_resolve_and_classify_xinput(u64* outRva, u32* outMethod) {
    ...
    if (!slot) {
        static constexpr u64 known[] = {0x1D0119E0ull,0x1C6109E0ull,0x1C5769E0ull,0x1EC409E0ull};
...
static DWORD WINABI worker_thread(void*) {
    ...
    load_config();
    {
        ...
        if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(b, '['); tb_time(b, st);
        tb_str(b, "] ACBFHapticsBridge v8-r9k-v2.3.7.3-ubisoft-isolated-fallback starting gameBase=0x"); tb_hex(b, (u64)g_gameBase);
    ...
    }
    {
        ...
        SYSTEMTIME st{};
        if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(b, '['); tb_time(b, st); tb_str(b, "] v8-r9k / v2.3.7.3 r9h-baseline + Ubisoft-isolated fallback; r9g behavior retained legacyWalkLoopMs=");
    ...
    log_simple("Verified target: Steam TU 1.0.7 SHA-256 614dab4a20a5d5c6256792e1daa6d05669c97a751079b10df1725d6965ad766d (flush/PostEvent via INI RVAs; dual-profile state hooks)");
    ...
    if (!install_v8_post_event_hook()) {
        // A moved/unknown PostEvent target is not fatal: the flush injector and
        // the input-driven fallback effects keep working with the semantic
        // router disabled. The site itself is never patched on a mismatch.
        g_cfg.semanticEvents = 0;
        log_simple("WARNING semantic router unavailable on this executable; continuing with input/flush paths only");
    }
    install_gameplay_state_hooks();
    log_simple("v8-r9k / v2.3.7.3 routing: v2.3.7.0/r9h gameplay, native-Quad haptics and conventional-rumble behavior retained; Steam/voices38 are baseline-only; Ubisoft Connect/Ubisoft+ may use a guarded 48kHz/4-channel DualSense fallback only when meaningful native Quad haptics are absent; XInputSetState import resolution is dynamic with verified-profile fallback");
    ...
    while (atomic_load_u32(&g_running)) {
        ...
        if (hotkey_pressed(VK_F9, f9Down) && atomic_load_u32(&g_enabled)) {
            schedule_configured_effect(g_map.dodge, 1.0f, g_cfg.fxDodge,
                                       ...
                                       g_cfg.dodgeStartTrimMs,
                                       g_cfg.semanticDodgeDurationMs);
            log_simple("F9 preview: v8-r9k semantic dodge trim/duration");
        }
