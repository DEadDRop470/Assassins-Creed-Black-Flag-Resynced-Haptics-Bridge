// ACBFHapticsBridge v8-r9i / v2.3.7.1 Ubisoft output-path hotfix
...
struct Config {
    ...
    // r9i: fallback renderer used only while Black Flag's native Quad sink is dormant.
    u32 directDualSenseFallback = 1;
    u32 directDualSenseFallbackDelayMs = 1000;
...
static constexpr u64 V8_POST_EVENT_RVA = 0x0091DEA0ull;
...
static void load_config() {
    ...
    g_cfg.directDualSenseFallback = (u32)read_ini_i32(
        L"Output", L"DirectDualSenseFallback", (i32)g_cfg.directDualSenseFallback);
    g_cfg.directDualSenseFallbackDelayMs = read_ini_u32_bounded(
        L"Output", L"DirectDualSenseFallbackDelayMs",
        g_cfg.directDualSenseFallbackDelayMs, 10000);
    if (g_cfg.directDualSenseFallbackDelayMs < 100)
        g_cfg.directDualSenseFallbackDelayMs = 100;
...
// r9i output ownership: 0=none, 1=Black Flag native Quad sink, 2=direct DualSense WaveOut.
static volatile u32 g_hapticOutputOwner{};
static volatile u32 g_r9iDirectActive{};
static volatile u32 g_r9iDirectDeviceId{};
static volatile u32 g_r9iDirectPeakPermille{};
static volatile u64 g_r9iDirectOpenAttempts{};
static volatile u64 g_r9iDirectOpenFailures{};
static volatile u64 g_r9iDirectWriteFailures{};
static volatile u64 g_r9iDirectBlocks{};
static volatile u64 g_r9iDirectNonZeroBlocks{};
static volatile u64 g_r9iDirectFrames{};
static u32 g_r9iAudioCancelSeen{};
static u64 g_workerStartTick{};
...
// v8-r9i / v2.3.7.1 direct DualSense 48 kHz fallback.
// Black Flag's native Quad/Wwise path always has priority. This renderer owns
// plugin haptics only after the native flush path has been silent for the
// configured grace period. A later native flush atomically takes ownership back.
#pragma pack(push,1)
struct R9I_WAVEFORMATEX {
...
struct R9I_GUID { u32 Data1; u16 Data2; u16 Data3; u8 Data4[8]; };
struct R9I_WAVEFORMATEXTENSIBLE {
    R9I_WAVEFORMATEX Format;
    union { WORD wValidBitsPerSample; WORD wSamplesPerBlock; WORD wReserved; } Samples;
    DWORD dwChannelMask; R9I_GUID SubFormat;
};
#pragma pack(pop)
static_assert(sizeof(R9I_WAVEFORMATEX) == 18, "r9i wave format");
static_assert(sizeof(R9I_WAVEFORMATEXTENSIBLE) == 40, "r9i extensible format");

struct R9I_WAVEHDR {
    char* lpData; DWORD dwBufferLength; DWORD dwBytesRecorded; usize dwUser;
    DWORD dwFlags; DWORD dwLoops; R9I_WAVEHDR* lpNext; usize reserved;
};
struct R9I_WAVEOUTCAPSW {
    WORD wMid, wPid; DWORD vDriverVersion; WCHAR szPname[32];
    DWORD dwFormats; WORD wChannels, wReserved1; DWORD dwSupport;
};
using R9I_HWAVEOUT = void*;
using R9I_MMRESULT = u32;
using R9I_GetNumFn = u32 (WINABI*)();
using R9I_GetCapsFn = R9I_MMRESULT (WINABI*)(usize, R9I_WAVEOUTCAPSW*, u32);
using R9I_OpenFn = R9I_MMRESULT (WINABI*)(R9I_HWAVEOUT*, usize,
    const R9I_WAVEFORMATEX*, usize, usize, DWORD);
using R9I_HeaderFn = R9I_MMRESULT (WINABI*)(R9I_HWAVEOUT, R9I_WAVEHDR*, u32);
using R9I_ResetFn = R9I_MMRESULT (WINABI*)(R9I_HWAVEOUT);
using R9I_CloseFn = R9I_MMRESULT (WINABI*)(R9I_HWAVEOUT);

static R9I_GetNumFn g_r9iGetNum{};
static R9I_GetCapsFn g_r9iGetCaps{};
static R9I_OpenFn g_r9iOpen{};
static R9I_HeaderFn g_r9iPrepare{}, g_r9iUnprepare{}, g_r9iWrite{};
static R9I_ResetFn g_r9iReset{};
static R9I_CloseFn g_r9iClose{};
static R9I_HWAVEOUT g_r9iWaveOut{};
static constexpr u32 R9I_DS_FRAMES = 480;   // 10 ms at 48 kHz
static constexpr u32 R9I_DS_BUFFERS = 3;    // bounded 30 ms queue
static constexpr DWORD R9I_WHDR_DONE = 1u;
static constexpr DWORD R9I_DS_MASK = 0x33u; // FL|FR|BL|BR; haptics are lanes 3/4
static constexpr WORD R9I_WAVE_FORMAT_EXTENSIBLE = 0xFFFEu;
struct R9I_DsBuf { R9I_WAVEHDR hdr{}; i16 pcm[R9I_DS_FRAMES * 4]{}; u32 prepared{}; };
static R9I_DsBuf g_r9iDsBuf[R9I_DS_BUFFERS]{};
static u64 g_r9iRetryTick{};

static bool r9i_name_contains(const WCHAR* s, const char* needle) {
    if (!s || !needle || !*needle) return false;
    char n[40]{};
    u32 nn = 0;
    while (needle[nn] && nn + 1 < (u32)sizeof(n)) {
        n[nn] = lower_ascii(needle[nn]);
        ++nn;
    }
    for (u32 i = 0; s[i] && i < 64u; ++i) {
        u32 j = 0;
        while (j < nn && s[i + j] &&
               lower_ascii((char)(s[i + j] & 0x7F)) == n[j]) ++j;
        if (j == nn) return true;
    }
    return false;
}
static bool r9i_dualsense_audio_name(const WCHAR* s) {
    return r9i_name_contains(s, "dualsense") ||
           r9i_name_contains(s, "wireless controller");
}
static bool r9i_init_wave() {
    if (g_r9iGetNum && g_r9iOpen && g_r9iWrite) return true;
    u8* module = find_module_ascii("winmm.dll");
    if (!module && g_api.LoadLibraryW)
        module = (u8*)g_api.LoadLibraryW(L"winmm.dll");
    if (!module) return false;
    g_r9iGetNum = api_cast<R9I_GetNumFn>(resolve_export(module, "waveOutGetNumDevs"));
    g_r9iGetCaps = api_cast<R9I_GetCapsFn>(resolve_export(module, "waveOutGetDevCapsW"));
    g_r9iOpen = api_cast<R9I_OpenFn>(resolve_export(module, "waveOutOpen"));
    g_r9iPrepare = api_cast<R9I_HeaderFn>(resolve_export(module, "waveOutPrepareHeader"));
    g_r9iUnprepare = api_cast<R9I_HeaderFn>(resolve_export(module, "waveOutUnprepareHeader"));
    g_r9iWrite = api_cast<R9I_HeaderFn>(resolve_export(module, "waveOutWrite"));
    g_r9iReset = api_cast<R9I_ResetFn>(resolve_export(module, "waveOutReset"));
    g_r9iClose = api_cast<R9I_CloseFn>(resolve_export(module, "waveOutClose"));
    return g_r9iGetNum && g_r9iGetCaps && g_r9iOpen && g_r9iPrepare &&
           g_r9iUnprepare && g_r9iWrite && g_r9iReset && g_r9iClose;
}
static i16 r9i_s16(float v) {
    v = clampf(v, -1.0f, 1.0f);
    i32 q = (i32)(v * 32767.0f);
    if (q < -32767) q = -32767;
    if (q > 32767) q = 32767;
    return (i16)q;
}
static float r9i_abs(float v) { return v < 0.0f ? -v : v; }

static bool r9i_haptic_work_pending() {
    return atomic_load_u32(&g_commandWrite) != atomic_load_u32(&g_commandRead) ||
           atomic_load_u32(&g_movementMode) != 0u;
}
static bool r9i_native_flush_recent(u64 now) {
    u64 last = atomic_load_u64(&g_lastAnySinkTick);
    if (!last || now < last) return false;
    return now - last < (u64)g_cfg.directDualSenseFallbackDelayMs;
}
static void r9i_render(R9I_DsBuf& buffer, u64 now) {
    float left[R9I_DS_FRAMES]{}, right[R9I_DS_FRAMES]{};
    u32 generation = atomic_load_u32(&g_cancelGeneration);
    if (generation != g_r9iAudioCancelSeen) {
        clear_playback();
        g_r9iAudioCancelSeen = generation;
    }
    drain_commands();
    if (atomic_load_u32(&g_enabled) && g_cfg.nativeHaptics && !native_feedback_active(now)) {
        mix_one_shots(left, right, R9I_DS_FRAMES);
        mix_movement(left, right, R9I_DS_FRAMES);
    } else if (native_feedback_active(now)) {
        clear_playback();
        atomic_fetch_add_u64(&g_nativeSuppressedHapticBlocks, 1);
    }
    float peak = 0.0f;
    const float limit = g_cfg.peakLimit;
    for (u32 i = 0; i < R9I_DS_FRAMES; ++i) {
        if (limit > 0.0f) {
            left[i] = clampf(left[i], -limit, limit);
            right[i] = clampf(right[i], -limit, limit);
        }
        float a = r9i_abs(left[i]);
        float b = r9i_abs(right[i]);
        if (b > a) a = b;
        if (a > peak) peak = a;
        buffer.pcm[i * 4u + 0u] = 0;              // normal audio left
        buffer.pcm[i * 4u + 1u] = 0;              // normal audio right
        buffer.pcm[i * 4u + 2u] = r9i_s16(left[i]);  // left actuator
        buffer.pcm[i * 4u + 3u] = r9i_s16(right[i]); // right actuator
    }
    if (peak > 0.0001f) {
        atomic_fetch_add_u64(&g_r9iDirectNonZeroBlocks, 1);
        u32 p = (u32)(clampf(peak, 0.0f, 100.0f) * 1000.0f);
        if (p > atomic_load_u32(&g_r9iDirectPeakPermille))
            atomic_store_u32(&g_r9iDirectPeakPermille, p);
    }
    buffer.hdr.lpData = (char*)buffer.pcm;
    buffer.hdr.dwBufferLength = (DWORD)sizeof(buffer.pcm);
}
static void r9i_direct_shutdown() {
    if (!g_r9iWaveOut) {
        atomic_store_u32(&g_r9iDirectActive, 0);
        if (atomic_load_u32(&g_hapticOutputOwner) == 2u)
            atomic_store_u32(&g_hapticOutputOwner, 0u);
        return;
    }
    if (g_r9iReset) g_r9iReset(g_r9iWaveOut);
    for (u32 i = 0; i < R9I_DS_BUFFERS; ++i) {
        if (g_r9iDsBuf[i].prepared && g_r9iUnprepare)
            g_r9iUnprepare(g_r9iWaveOut, &g_r9iDsBuf[i].hdr, (u32)sizeof(R9I_WAVEHDR));
        g_r9iDsBuf[i].prepared = 0;
    }
    if (g_r9iClose) g_r9iClose(g_r9iWaveOut);
    g_r9iWaveOut = nullptr;
    atomic_store_u32(&g_r9iDirectActive, 0);
    if (atomic_load_u32(&g_hapticOutputOwner) == 2u)
        atomic_store_u32(&g_hapticOutputOwner, 0u);
}
static bool r9i_direct_open(u64 now) {
    atomic_fetch_add_u64(&g_r9iDirectOpenAttempts, 1);
    if (!r9i_init_wave()) {
        atomic_fetch_add_u64(&g_r9iDirectOpenFailures, 1);
        return false;
    }
    R9I_WAVEFORMATEXTENSIBLE fmt{};
    fmt.Format.wFormatTag = R9I_WAVE_FORMAT_EXTENSIBLE;
    ...
    fmt.dwChannelMask = R9I_DS_MASK;
    fmt.SubFormat.Data1 = 1;      // KSDATAFORMAT_SUBTYPE_PCM
    fmt.SubFormat.Data2 = 0;
    fmt.SubFormat.Data3 = 0x0010;
    const u8 tail[8] = {0x80,0x00,0x00,0xAA,0x00,0x38,0x9B,0x71};
    memcpy(fmt.SubFormat.Data4, tail, 8);

    u32 count = g_r9iGetNum();
    for (u32 dev = 0; dev < count; ++dev) {
        R9I_WAVEOUTCAPSW caps{};
        if (g_r9iGetCaps((usize)dev, &caps, (u32)sizeof(caps)) != 0) continue;
        if (!r9i_dualsense_audio_name(caps.szPname)) continue;
        R9I_HWAVEOUT out{};
        // Exact-format open is the capability check. Do not trust caps.wChannels:
        // some virtual/controller endpoints report legacy caps incompletely.
        if (g_r9iOpen(&out, (usize)dev, &fmt.Format, 0, 0, 0) != 0 || !out) continue;

        g_r9iWaveOut = out;
        atomic_store_u32(&g_r9iDirectDeviceId, dev + 1u); // 0 means none
        atomic_store_u32(&g_r9iDirectActive, 1);
        atomic_store_u32(&g_hapticOutputOwner, 2u);
        bool ok = true;
        for (u32 i = 0; i < R9I_DS_BUFFERS; ++i) {
            g_r9iDsBuf[i] = {};
            r9i_render(g_r9iDsBuf[i], now);
            if (g_r9iPrepare(out, &g_r9iDsBuf[i].hdr, (u32)sizeof(R9I_WAVEHDR)) != 0) {
                ok = false; break;
            }
            g_r9iDsBuf[i].prepared = 1;
            if (g_r9iWrite(out, &g_r9iDsBuf[i].hdr, (u32)sizeof(R9I_WAVEHDR)) != 0) {
                ok = false; break;
            }
            atomic_fetch_add_u64(&g_r9iDirectBlocks, 1);
            atomic_fetch_add_u64(&g_r9iDirectFrames, R9I_DS_FRAMES);
        }
        if (!ok) {
            atomic_fetch_add_u64(&g_r9iDirectWriteFailures, 1);
            r9i_direct_shutdown();
            continue;
        }
        TextBuf log{}; SYSTEMTIME st{};
        if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(log, '['); tb_time(log, st);
        tb_str(log, "] R9I direct DualSense haptics ACTIVE device="); tb_u64(log, dev);
        tb_str(log, " name=");
        for (u32 k = 0; k < 32u && caps.szPname[k]; ++k) {
            WCHAR w = caps.szPname[k];
            tb_char(log, (w >= 32 && w < 127) ? (char)w : '?');
        }
        tb_str(log, " 48000Hz/4ch/16bit actuatorChannels=3/4");
        tb_endline(log); append_log(log.bytes, log.len);
        return true;
    }
    atomic_fetch_add_u64(&g_r9iDirectOpenFailures, 1);
    return false;
}
static void service_direct_dualsense_haptics(u64 now) {
    if (!g_cfg.directDualSenseFallback || !g_cfg.nativeHaptics ||
        !atomic_load_u32(&g_enabled) || !atomic_load_u32(&g_activeDualSense)) {
        r9i_direct_shutdown();
        return;
    }
    // Any recently executing Black Flag Quad flush owns plugin haptics.
    if (r9i_native_flush_recent(now)) {
        atomic_store_u32(&g_hapticOutputOwner, 1u);
        r9i_direct_shutdown();
        return;
    }
    if (!g_workerStartTick ||
        now < g_workerStartTick + (u64)g_cfg.directDualSenseFallbackDelayMs) return;

    if (!g_r9iWaveOut) {
        // Stay completely dormant until an effect actually needs rendering.
        if (!r9i_haptic_work_pending()) return;
        if (now < g_r9iRetryTick) return;
        g_r9iRetryTick = now + 2000ull;
        if (!r9i_direct_open(now)) {
            if (atomic_load_u64(&g_r9iDirectOpenFailures) == 1u)
                log_simple("WARNING r9i could not open a 48kHz 4-channel DualSense/Wireless Controller audio endpoint; retrying on later haptic work");
            return;
        }
    }
    if (r9i_native_flush_recent(now)) {
    ...
    for (u32 i = 0; i < R9I_DS_BUFFERS; ++i) {
        R9I_DsBuf& buffer = g_r9iDsBuf[i];
        if (!buffer.prepared || !(buffer.hdr.dwFlags & R9I_WHDR_DONE)) continue;
        if (g_r9iUnprepare(g_r9iWaveOut, &buffer.hdr, (u32)sizeof(R9I_WAVEHDR)) != 0)
            continue;
        buffer.prepared = 0;
        buffer.hdr = {};
        r9i_render(buffer, now);
        if (g_r9iPrepare(g_r9iWaveOut, &buffer.hdr, (u32)sizeof(R9I_WAVEHDR)) != 0) {
            atomic_fetch_add_u64(&g_r9iDirectWriteFailures, 1);
            r9i_direct_shutdown();
            return;
        }
        buffer.prepared = 1;
        if (g_r9iWrite(g_r9iWaveOut, &buffer.hdr, (u32)sizeof(R9I_WAVEHDR)) != 0) {
            atomic_fetch_add_u64(&g_r9iDirectWriteFailures, 1);
            r9i_direct_shutdown();
            return;
        }
...
static void FASTABI flush_hook(void* self) {
    ...
    u32 previousOutputOwner = atomic_exchange_u32(&g_hapticOutputOwner, 1u);
    ...
    if (sink && sink == primary && previousOutputOwner != 2u) inject_sink_block(self);
...
static bool install_v8_post_event_hook() {
    ...
    if (!g_gameBase || V8_POST_EVENT_RVA + 6 > g_gameSize) return false;
    static const u8 signature[6] = {0x41,0x57,0x41,0x56,0x41,0x55};
    u8* target = g_gameBase + V8_POST_EVENT_RVA;
    ...
    log_simple("Installed v8 exact Wwise PostEvent router at RVA 0x0091DEA0");
    return true;
}
...
static volatile u64 g_r9iGameInputRumbleAttempts{};
static volatile u64 g_r9iXInputRumbleAttempts{};
static volatile u64 g_r9iXInputRumbleFailures{};
static volatile u64 g_r9iXInputRouteProbes{};
static volatile u32 g_r9iXInputRouteAvailable{};
static u64 g_r9iXInputRouteProbeTick{};
...
// v8-r9i / v2.3.7.1: dynamic XInputSetState IAT resolution.
static bool r9i_game_rva_valid(u64 rva, u64 size) {
    return g_gameBase && rva < (u64)g_gameSize && size <= (u64)g_gameSize - rva;
}
static bool r9i_ascii_starts_ci(const char* value, const char* prefix) {
...
static bool r9i_ascii_equal(const char* a, const char* b) {
    if (!a || !b) return false;
    while (*a && *b) if (*a++ != *b++) return false;
    return *a == 0 && *b == 0;
}
static u64* r9i_find_xinput_setstate_iat(u64* outRva, u32* outMethod) {
    if (outRva) *outRva = 0;
    if (outMethod) *outMethod = 0;
    if (!g_gameBase || !g_xinputSetState || !r9i_game_rva_valid(0, 0x40) ||
        *(u16*)g_gameBase != 0x5A4D) return nullptr;

    u32 peoff = *(u32*)(g_gameBase + 0x3C);
    if (!r9i_game_rva_valid(peoff, 24)) return nullptr;
    ...
    if (!importRva || !r9i_game_rva_valid(importRva, 20)) return nullptr;
    ...
    for (u32 d = 0; d < descriptors; ++d) {
        ...
        if (!r9i_game_rva_valid(descRva, 20)) break;
        ...
        if (!nameRva || !firstThunk || !r9i_game_rva_valid(nameRva, 1) ||
            !r9i_game_rva_valid(firstThunk, 8)) continue;
        const char* dll = (const char*)(g_gameBase + nameRva);
        if (!r9i_ascii_starts_ci(dll, "xinput")) continue;

        for (u32 i = 0; i < 4096u; ++i) {
            u64 slotRva = (u64)firstThunk + (u64)i * sizeof(u64);
            if (!r9i_game_rva_valid(slotRva, sizeof(u64))) break;
            u64* slot = (u64*)(g_gameBase + slotRva);
            u64 target = *slot;
            if (!target) break;
            ...
            if (oft) {
                ...
                if (!r9i_game_rva_valid(intRva, sizeof(u64))) break;
                ...
                if (!(nameThunk & ORD64) && nameThunk <= 0xFFFFFFFFull &&
                    r9i_game_rva_valid(nameThunk, 3)) {
                    u64 symRva = nameThunk + 2ull;
                    if (r9i_game_rva_valid(symRva, 15)) {
                        const char* sym = (const char*)(g_gameBase + symRva);
                        if (r9i_ascii_equal(sym, "XInputSetState")) {
...
static void install_game_xinput_observer() {
    if (!g_cfg.nativeArbitration || !g_xinputSetState || !g_gameBase) return;
    u64 slotRva = 0;
    u32 method = 0;
    u64* slot = r9i_find_xinput_setstate_iat(&slotRva, &method);

    if (!slot) {
        static constexpr u64 known[] = {
            0x1D0119E0ull, // Steam TU 1.0.6 / voices38
            0x1C5769E0ull, // Ubisoft Connect
            0x1EC409E0ull  // Ubisoft+
        };
        for (u32 i = 0; i < (u32)(sizeof(known) / sizeof(known[0])); ++i) {
            if (!r9i_game_rva_valid(known[i], sizeof(u64))) continue;
            u64* candidate = (u64*)(g_gameBase + known[i]);
            if (*candidate == (u64)g_xinputSetState) {
                slot = candidate; slotRva = known[i]; method = 3; break;
            }
        }
    }
    if (!slot || !*slot) {
        atomic_fetch_add_u32(&g_gameXInputObserverFailures, 1);
        log_simple("WARNING r9i XInput observer could not resolve verified XInputSetState IAT");
        return;
    }
    ...
    if (!g_api.VirtualProtect || !g_api.VirtualProtect(slot, sizeof(u64), PAGE_READWRITE, &oldProtect)) {
        g_gameXInputSetStateOriginal = nullptr;
        atomic_fetch_add_u32(&g_gameXInputObserverFailures, 1);
        log_simple("WARNING r9i XInput observer could not make IAT writable");
        return;
    }
    *slot = (u64)&game_xinput_set_state_observer;
    ...
    tb_str(b,"] Installed r9i XInput observer IAT RVA 0x"); tb_hex(b,slotRva);
    tb_str(b," method="); tb_str(b,method==1?"named-import":method==2?"pointer-match":"profile-fallback");
...
static void update_gameinput_device(void* device, u64 now) {
    ...
    if (!changed) {
        ...
        atomic_store_u32(&g_activeDualSense,
                         is_dualsense_product(vendor, product) ? 1u : 0u);
...
static bool set_gameinput_rumble(float low, float high) {
    atomic_fetch_add_u64(&g_r9iGameInputRumbleAttempts, 1);
    ...
    // GameInput v0 returns void. This counter means "command issued", not
    // verified physical vibration; r9i therefore also measures XInput writes.
...
static bool set_xinput_rumble(float low, float high) {
    atomic_fetch_add_u64(&g_r9iXInputRumbleAttempts, 1);
    if (!g_xinputSetState) {
        atomic_fetch_add_u64(&g_r9iXInputRumbleFailures, 1);
        return false;
    }
    XINPUT_VIBRATION vibration{};
    ...
    else atomic_fetch_add_u64(&g_r9iXInputRumbleFailures, 1);
    return wrote;
}
// v8-r9i: GameInput v0 SetRumbleState returns void. Issuing the COM call is
// therefore not proof that the physical/virtual device accepted vibration.
// Probe XInput periodically and mirror the motor state when that route exists.
static bool xinput_rumble_route_available(u64 now) {
    if (!g_xinputGetState || !g_xinputSetState) {
        atomic_store_u32(&g_r9iXInputRouteAvailable, 0);
        return false;
    }
    if (!g_r9iXInputRouteProbeTick || now < g_r9iXInputRouteProbeTick ||
        now - g_r9iXInputRouteProbeTick >= 500ull) {
        g_r9iXInputRouteProbeTick = now;
        XINPUT_STATE state{};
        atomic_fetch_add_u64(&g_r9iXInputRouteProbes, 1);
        bool available = g_xinputGetState(g_cfg.controllerIndex, &state) == ERROR_SUCCESS;
        atomic_store_u32(&g_r9iXInputRouteAvailable, available ? 1u : 0u);
    }
    return atomic_load_u32(&g_r9iXInputRouteAvailable) != 0u;
}
...
static void write_rumble_output(const RumbleSample& sample) {
    ...
    if (native_rumble_active(now)) {
        // The game's write already replaced the motor state owned by the bridge.
        // Do not issue a competing zero that would cancel the native effect.
        g_rumbleOutputWasActive = false;
        atomic_store_u32(&g_rumbleBackend, 0);
        return;
    }
    if (native_haptic_active(now)) {
        // Native audio haptics do not necessarily overwrite conventional motors.
        // Clear only a state that this bridge previously owned.
        if (g_rumbleOutputWasActive) {
            if (atomic_load_u32(&g_inputBackend) == 1 && g_gameInputDevice)
                set_gameinput_rumble(0.0f, 0.0f);
            if (!atomic_load_u32(&g_activeDualSense) ||
                xinput_rumble_route_available(now))
    ...
    if (atomic_load_u32(&g_activeDualSense) && !g_cfg.rumbleOnDualSense) {
        if (g_rumbleOutputWasActive) {
            if (atomic_load_u32(&g_inputBackend) == 1 && g_gameInputDevice)
                set_gameinput_rumble(0.0f, 0.0f);
            if (xinput_rumble_route_available(now))
                set_xinput_rumble(0.0f, 0.0f);
        }
        g_rumbleOutputWasActive = false;
        atomic_store_u32(&g_rumbleBackend, 0);
    ...
    bool dualSense = atomic_load_u32(&g_activeDualSense) != 0u;
    bool xinputRoute = xinput_rumble_route_available(now);
    if (backend == 1 && g_gameInputDevice)
        gameInputWrote = set_gameinput_rumble(low, high);

    // r9i fix: a GameInput void call is no longer treated as proof of output.
    // For a DualSense, mirror through XInput whenever an actual XInput route is
    // connected (DSX/Steam Input/ViGEm scenarios). Non-DualSense behavior keeps
    // the existing mirror/fallback semantics.
    if (!dualSense || xinputRoute || !gameInputWrote)
...
static void log_r9i_output_status(u64 now) {
    TextBuf b{}; SYSTEMTIME st{};
    if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
    tb_char(b, '['); tb_time(b, st); tb_str(b, "] R9I OUTPUT owner=");
    u32 owner = atomic_load_u32(&g_hapticOutputOwner);
    tb_str(b, owner == 1u ? "native-quad" : owner == 2u ? "direct-dualsense" : "none");
    tb_str(b, " nativeFlushRecent="); tb_u64(b, r9i_native_flush_recent(now) ? 1u : 0u);
    tb_str(b, " directDS=active/openTry/openFail/writeFail/blocks/nonZero/frames/device/peakP=");
    tb_u64(b, atomic_load_u32(&g_r9iDirectActive)); tb_char(b, '/');
    tb_u64(b, atomic_load_u64(&g_r9iDirectOpenAttempts)); tb_char(b, '/');
    tb_u64(b, atomic_load_u64(&g_r9iDirectOpenFailures)); tb_char(b, '/');
    tb_u64(b, atomic_load_u64(&g_r9iDirectWriteFailures)); tb_char(b, '/');
    tb_u64(b, atomic_load_u64(&g_r9iDirectBlocks)); tb_char(b, '/');
    tb_u64(b, atomic_load_u64(&g_r9iDirectNonZeroBlocks)); tb_char(b, '/');
    tb_u64(b, atomic_load_u64(&g_r9iDirectFrames)); tb_char(b, '/');
    tb_u64(b, atomic_load_u32(&g_r9iDirectDeviceId)); tb_char(b, '/');
    tb_u64(b, atomic_load_u32(&g_r9iDirectPeakPermille));
    tb_str(b, " rumble=giTry/giIssued/xiTry/xiOK/xiFail/route/probes=");
    tb_u64(b, atomic_load_u64(&g_r9iGameInputRumbleAttempts)); tb_char(b, '/');
    tb_u64(b, atomic_load_u64(&g_gameInputRumbleWrites)); tb_char(b, '/');
    tb_u64(b, atomic_load_u64(&g_r9iXInputRumbleAttempts)); tb_char(b, '/');
    tb_u64(b, atomic_load_u64(&g_xinputRumbleWrites)); tb_char(b, '/');
    tb_u64(b, atomic_load_u64(&g_r9iXInputRumbleFailures)); tb_char(b, '/');
    tb_u64(b, atomic_load_u32(&g_r9iXInputRouteAvailable)); tb_char(b, '/');
    tb_u64(b, atomic_load_u64(&g_r9iXInputRouteProbes));
    tb_endline(b); append_log(b.bytes, b.len);
}
...
static DWORD WINABI worker_thread(void*) {
    ...
    load_config();
    {
        ...
        if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(b, '['); tb_time(b, st);
        tb_str(b, "] ACBFHapticsBridge v8-r9i-v2.3.7.1-ubisoft-output-hotfix starting gameBase=0x"); tb_hex(b, (u64)g_gameBase);
        ...
        tb_str(b, " directDSFallback/delayMs="); tb_u64(b, g_cfg.directDualSenseFallback);
        tb_char(b, '/'); tb_u64(b, g_cfg.directDualSenseFallbackDelayMs);
        tb_endline(b);
        append_log(b.bytes, b.len);
    }
    {
        ...
        SYSTEMTIME st{};
        if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(b, '['); tb_time(b, st); tb_str(b, "] v8-r9i / v2.3.7.1 Ubisoft output hotfix; r9g behavior retained legacyWalkLoopMs=");
    ...
    {
        TextBuf b{}; SYSTEMTIME st{}; if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(b,'['); tb_time(b,st); tb_str(b,"] V8R9ICFG semanticEvents/menu/gaitHistory/attackProbe/oneShotProbe/airTimeout=");
    ...
    log_simple("v8-r9i / v2.3.7.1 routing: r9g gameplay/tuning retained; native Quad sink remains first choice; dormant-sink DualSense 48kHz fallback added; GameInput void rumble writes are no longer treated as proof of physical success; connected XInput routes are mirrored and measured; XInput observer now resolves the PE import dynamically with verified-profile fallback");
    ...
    g_workerStartTick = lastRumbleTick;
    ...
    while (atomic_load_u32(&g_running)) {
        ...
        if (hotkey_pressed(VK_F9, f9Down) && atomic_load_u32(&g_enabled)) {
            schedule_configured_effect(g_map.dodge, 1.0f, g_cfg.fxDodge,
                                       ...
                                       g_cfg.dodgeStartTrimMs,
                                       g_cfg.semanticDodgeDurationMs);
            log_simple("F9 preview: v8-r9i semantic dodge trim/duration + output fallback");
        }
        if (hotkey_pressed(VK_F10, f10Down)) { write_status(); log_r9i_output_status(loopNow); }
        ...
        service_direct_dualsense_haptics(now);
