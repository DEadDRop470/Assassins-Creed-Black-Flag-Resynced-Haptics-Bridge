// ACBFHapticsBridge v8-r4 hybrid semantic/gameplay-state revision
...
struct Config {
    ...
    // r4 phase timing and water/run behavior.
    u32 heavyPhase1StartMs = 120;
    u32 heavyPhase1DurationMs = 180;
    u32 heavyPhase2DelayMs = 100;
    ...
    float dualSenseWalkHapticMultiplier = 4.0f;
    float runBaseGain = 1.50f;
    float runOpeningGain = 1.90f;
...
static bool schedule_split_arbitrated_effect(i32 effect, float hapticGain,
    ...
    if (!g_cfg.nativeArbitration || !delayMs) {
        bool h = g_cfg.nativeHaptics ?
            queue_haptic_effect(effect, hapticGain, source, startFrame, maxFrames) : false;
...
static void FASTABI flush_hook(void* self) {
    ...
    if (resumed || rebound) {
        ...
        // v8-r4 separates output recovery from gameplay-domain recovery. A
        // harmless sink replacement no longer erases MENU/CHARACTER/WATER/HELM.
        // Long silence/resume still means load/desync/reload and resets domain.
        u32 cause = (resumed ? 1u : 0u) | (rebound ? 2u : 0u);
        atomic_fetch_or_u32(&g_recoveryCause, cause);
...
static void process_gameplay_state_signals(u64 now) {
    ...
    for (u32 event = 0; event < GSE_COUNT; ++event) {
        ...
        case GSE_CLIMB_ENTER:
            ...
            break;
        case GSE_SWIM_ENTER:
            // Being accepted into Edward's Swimming state is real evidence that
            // helm control ended, including a knock-off into the sea.
            if (g_cfg.semanticEvents &&
                atomic_load_u32(&g_v8RuntimeMode) == V8_MODE_HELM) {
                atomic_fetch_add_u64(&g_v8HelmReleases, 1);
...
static void process_controller_edges(const XINPUT_STATE& current,
    ...
    if (!g_cfg.semanticEvents) {
        ...
        if (attackFalling && g_r4AttackPressActive) {
            ...
            bool semanticOwned = sem && sem >= g_r4AttackPressTick && sem <= now + 40;
            if (!semanticOwned && r4Mode == V8_MODE_CHARACTER) {
...
static void v8_schedule_gait_pulse(const V8SemanticEvent& ev, bool walk,
    ...
    float dynamicScale = movement_dynamic_scale();
    float hapticGain = g_cfg.movementGain * dynamicScale *
        control.intensityMultiplier *
        (walk ? g_cfg.dualSenseWalkHapticMultiplier : 1.0f);
    ...
    if (hapticGain > 16.0f) hapticGain = 16.0f;
...
static void r4_schedule_heavy_phases(u64 now, u64 nativeSerialBaseline, u32 baseSource) {
    // The Shadows heavy capture's strongest early content is around 0.12-0.34 s.
    // Split it into two compact phases so Black Flag does not inherit the
    // capture's long internal timing. Phase 1 is stronger; phase 2 is advanced.
    ...
    if (schedule_configured_effect(g_map.heavy, 1.00f, g_cfg.fxHeavySecond,
                                   baseSource + 1u, now,
                                   g_cfg.semanticAttackProbeMs + g_cfg.heavyPhase2DelayMs,
                                   nativeSerialBaseline,
                                   g_cfg.heavyPhase2StartMs,
                                   g_cfg.heavyPhase2DurationMs))
...
static void v8_handle_semantic_event(const V8SemanticEvent& ev) {
    ...
    case EVT_PISTOL_FIRE:
        // Perfect-parry and pistol native feedback remain untouched.
        if (!v8_enter_character_from_event(ev, now, true)) break;
        atomic_fetch_add_u64(&g_v8SemanticNativeOnly, 1);
        break;
    case EVT_PLAYER_HIT:
...
static DWORD WINABI worker_thread(void*) {
    ...
    load_config();
    {
        ...
        if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(b, '['); tb_time(b, st);
        tb_str(b, "] ACBFHapticsBridge v8-r4-hybrid-state-combo-water starting gameBase=0x"); tb_hex(b, (u64)g_gameBase);
    ...
    }
    {
        ...
        SYSTEMTIME st{};
        if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(b, '['); tb_time(b, st); tb_str(b, "] v8-r4 semantic/state hybrid + phase/run/water tuning legacyWalkLoopMs=");
    ...
    log_input_bindings();
    {
        ...
        tb_char(b,'['); tb_time(b,st); tb_str(b,"] V8R4CFG semanticEvents/menu/gaitHistory/attackProbe/oneShotProbe/airTimeout=");
    ...
    log_simple("v8-r4 routing: gameplay-state airtime; 24ms normal-parry fallback; hybrid per-R1 combo fallback; two-phase heavy/sheath; L3 continuous land run + 3s underwater lunge cap; native cannon/player-hit/pistol/perfect-parry untouched");
    ...
    while (atomic_load_u32(&g_running)) {
        ...
        if (hotkey_pressed(VK_F9, f9Down) && atomic_load_u32(&g_enabled)) {
            schedule_configured_effect(g_map.dodge, 1.0f, g_cfg.fxDodge,
                                       ...
                                       g_cfg.dodgeStartTrimMs,
                                       g_cfg.semanticDodgeDurationMs);
            log_simple("F9 preview: v8-r4 semantic dodge trim/duration");
        }
