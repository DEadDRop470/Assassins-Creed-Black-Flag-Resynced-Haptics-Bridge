// ACBFHapticsBridge v8-r9h / v2.3.7.0 verified multi-version compatibility revision
...
static void install_game_xinput_observer() {
    // Verified ACBlackFlag.exe profiles keep the same game/runtime code but
    // place the XInputSetState import slot at different RVAs. Probe only the
    // known profile RVAs and require the slot to equal the resolved API export
    // before patching it. This keeps the observer fail-closed on unknown builds.
    static constexpr u64 kXInputSetStateIatRvas[] = {
        0x1D0119E0ull, // Steam TU 1.0.6 + voices38 profile
        0x1C5769E0ull, // Ubisoft Connect profile
        0x1EC409E0ull  // Ubisoft+ protected profile
    };
    if (!g_cfg.nativeArbitration || !g_xinputSetState || !g_gameBase) return;

    u64* slot = nullptr;
    u64 slotRva = 0;
    for (u32 i = 0; i < (u32)(sizeof(kXInputSetStateIatRvas) / sizeof(kXInputSetStateIatRvas[0])); ++i) {
        u64 candidateRva = kXInputSetStateIatRvas[i];
        if (candidateRva + sizeof(u64) > g_gameSize) continue;
        u64* candidate = (u64*)(g_gameBase + candidateRva);
        if (*candidate == (u64)g_xinputSetState) {
            slot = candidate;
            slotRva = candidateRva;
            break;
        }
    }
    if (!slot) {
        atomic_fetch_add_u32(&g_gameXInputObserverFailures, 1);
        log_simple("WARNING native XInput observer IAT verification failed for all verified profiles; leaving game imports untouched");
        return;
    }
    DWORD oldProtect = 0;
    if (!g_api.VirtualProtect(slot, sizeof(u64), PAGE_READWRITE, &oldProtect)) {
    ...
    *slot = (u64)&game_xinput_set_state_observer;
    ...
    TextBuf b{};
    tb_str(b, "Installed verified game XInputSetState native-rumble observer at IAT RVA 0x");
    tb_hex(b, slotRva);
    tb_endline(b);
    append_log(b.bytes, b.len);
}
...
static DWORD WINABI worker_thread(void*) {
    ...
    load_config();
    {
        ...
        if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(b, '['); tb_time(b, st);
        tb_str(b, "] ACBFHapticsBridge v8-r9h-v2.3.7.0-multiversion starting gameBase=0x"); tb_hex(b, (u64)g_gameBase);
    ...
    }
    {
        ...
        SYSTEMTIME st{};
        if (g_api.GetLocalTime) g_api.GetLocalTime(&st);
        tb_char(b, '['); tb_time(b, st); tb_str(b, "] v8-r9h / v2.3.7.0 multi-version compatibility; r9g behavior retained legacyWalkLoopMs=");
    ...
    log_input_bindings();
    {
        ...
        tb_char(b,'['); tb_time(b,st); tb_str(b,"] V8R9FCFG semanticEvents/menu/gaitHistory/attackProbe/oneShotProbe/airTimeout=");
    ...
    log_simple("v8-r9h / v2.3.7.0 routing: r9g behavior/tuning retained; verified XInput observer profiles added for Steam/voices38, Ubisoft Connect, and Ubisoft+; input worker remains 2 ms (500 Hz); all prior semantic, state, native-haptic, rumble, menu and performance behavior retained");
    ...
    while (atomic_load_u32(&g_running)) {
        ...
        if (hotkey_pressed(VK_F9, f9Down) && atomic_load_u32(&g_enabled)) {
            schedule_configured_effect(g_map.dodge, 1.0f, g_cfg.fxDodge,
                                       ...
                                       g_cfg.dodgeStartTrimMs,
                                       g_cfg.semanticDodgeDurationMs);
            log_simple("F9 preview: v8-r9h semantic dodge trim/duration");
        }
