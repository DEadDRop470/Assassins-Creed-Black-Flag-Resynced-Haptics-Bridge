...
<p align="center">
  <img src="https://img.shields.io/badge/RELEASE-v2.3.7.5-76C893?style=for-the-badge" alt="Release v2.3.7.5">
  <img src="https://img.shields.io/badge/BUILD-v8--r9m-7FB3FF?style=for-the-badge" alt="Build v8-r9m">
...
<p align="center">
  <strong>Runtime:</strong> <code>v8-r9m-v2.3.7.5-1.06-1.07-compat</code>
</p>
---
> 🟠 \*\*IMPORTANT\*\*
...
The current v2.3.7.5 / v8-r9m build deliberately uses the proven v2.3.7.0 / v8-r9h behavior as its gameplay, Steam DualSense haptic and conventional-rumble baseline.
...
For Ubisoft Connect / Ubisoft+, Black Flag's native Quad haptic path remains the first choice. If meaningful native Quad haptics are absent while bridge HFX playback is required, v8-r9m can use a guarded 48 kHz / 4-channel direct DualSense fallback on actuator channels 3/4.
The mod supports:
...
> 🔵 \*\*NOTE\*\*
> The verified v2.3.7.5 bridge primarily uses \*\*GameInput and XInput-compatible controller paths\*\*. Generic DirectInput support should not be assumed for every device.
> 🔵 \*\*NOTE\*\*
> \*\*DUALSENSE HAPTICS VS RUMBLE:\*\* The 48 kHz DualSense haptic stream and conventional rumble are \*\*parallel outputs\*\*. A strong haptic effect does not automatically "overflow" into rumble. The haptic path is limited by \*\*PeakLimit\*\*, while conventional rumble is generated separately when enabled. On Steam, v8-r9m preserves the proven v2.3.7.0 / r9h routing behavior.
✅ Confirmed / Supported Game Versions
Version	Status
Steam — Title Update 1.0.6	✅ Supported — v2.3.7.0 / r9h is the confirmed working behavioral baseline used by r9m
Supplied Title Update 1.0.7 executable	Hook addresses and imports verified; startup fixes included
...
v2.3.7.5 retains dynamic XInputSetState PE-import resolution, with verified executable-specific locations retained only as pointer-validated fail-closed fallbacks.
The important difference in v8-r9m is that the newer output compatibility logic is isolated to Ubisoft Connect / Ubisoft+.
Steam keeps the known-good v2.3.7.0 / r9h output behavior.
For Ubisoft profiles, r9m also:
...
✨ What the ASI mod does — v2.3.7.5
...
🆕 v2.3.7.5 / v8-r9m — Startup Compatibility Fix
Repairs v2.3.7.4 startup discovery: memory scans skip inaccessible pages, verified 1.07 native addresses are checked directly, and the layout is retained before installing hooks so all 21 correct gameplay-state targets are selected. Original hook pointers are prepared before entry-point patches are published. Static and guard-page regression checks passed; Windows game/controller validation is pending. Haptics and rumble configuration, intensity and output routines are unchanged.
Retained output behavior:
v2.3.7.5 deliberately returns to v2.3.7.0 / v8-r9h as the behavioral baseline.
...
> 🔵 \*\*NOTE\*\*
> \*\*In short:\*\* v2.3.7.5 keeps the proven v2.3.7.0 Steam haptic behavior and adds the newer compatibility work only where it is needed: Ubisoft Connect / Ubisoft+.
...
F10: Writes a detailed runtime snapshot to ACBFHapticsBridge-v8.log, including controller/backend information, semantic/gameplay state, water/landing diagnostics, native arbitration, effect activity and v8-r9m diagnostics.
...
```
🧩 Special v2.3.7.5 Effect Settings
...
MasterGain=0.90 = supplied v2.3.7.5 master setting, equal to 90% of the normalized tuned master baseline
...
BEffect=dodge
LBEffect=parry
RBEffect=light\_attack\_single
XEffect=parry
```
...
🔐 Verified v2.3.7.5 Build
Item	Value
Release	`v2.3.7.5`
Internal build	`ACBFHapticsBridge-v8-r9m`
Runtime label	`v8-r9m-v2.3.7.5-1.06-1.07-compat`
ASI	`ACBFHapticsBridge-v8-r9m.asi`
ASI SHA-256
```text
...
```
Two clean consecutive r9m builds produced the same ASI byte-for-byte.
...
> 🔵 \*\*NOTE\*\*
> v2.3.7.0 / r9h remains the confirmed working Steam behavioral baseline. v2.3.7.5 / r9m is deliberately built around that baseline while isolating the newer compatibility work to Ubisoft profiles.
📂 Source Notice — v2.3.7.5
The current v2.3.7.5 release contains the actual buildable source used for the current ASI.
A simplified layout is:
```text
Source/
├─ src/
│  └─ main.cpp
├─ build\_linux\_pe\_r9m.sh
├─ audit\_r9m.py
├─ verify\_r9h\_baseline.py
├─ validate\_hfx.py
├─ ACBFHapticsBridge-v8.ini
├─ SOURCE-CHECKSUMS.sha256
├─ BUILD-VERIFICATION.md
├─ R9K-IMPLEMENTATION.md
├─ SOURCE-DELTA-AUDIT-v8-r9m.md
├─ UBISOFT-CROSSREFERENCE.md
├─ TEST-CHECKLIST.md
├─ CHANGELOG-v2.3.7.0-to-v2.3.7.5.md
├─ ACBFHapticsBridge-v2.3.7.5-NONCOMMERCIAL-LICENSE.txt
├─ LICENSE.txt
└─ release/build provenance and historical change notes
```
📜 License — Non-Commercial
> 🟠 \*\*IMPORTANT\*\*
> \*\*ACBFHapticsBridge v2.3.7.5 is NOT released under the MIT License.\*\*  
...
The current v2.3.7.5 package includes:
```text
version.dll
```
